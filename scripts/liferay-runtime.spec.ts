...
test('registers every Nexcent custom element on a real Liferay page', async ({
    ...
    const registered = await page.evaluate((names) =>
        names.filter((name) => Boolean(customElements.get(name)))
    , expectedElements);
...
test('renders a production section inside the Liferay Shadow DOM boundary', async ({
    ...
    await page.evaluate(
        ({logoUrl}) => {
            ...
            host.setAttribute('description', 'Rendered by the deployed client extension.');
        ...
        .poll(
            () =>
                page.evaluate(() => {
                    ...
                    return host?.shadowRoot?.querySelectorAll(
                        '.clients img'
                    ).length ?? 0;
                }),
            {timeout: 30_000}
            ...
            page.evaluate(() =>
                Boolean(
                    document
                        ...
                        ?.shadowRoot?.querySelector('.header__account-trigger')
                )
            )
        )
        .toBe(true);
    ...
    await page.evaluate(() => {
        ...
        trigger.setAttribute(
            ...
            JSON.stringify({
                id: 'runtime-modal',
                slots: {
                    description: {value: 'Opened inside a deployed Liferay client extension.'},
